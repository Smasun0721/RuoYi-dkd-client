  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="empRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="修改时间:" prop="updateTime" v-if="title === '修改人员'">
          {{ form.updateTime }}
        </el-form-item>
        ...
        <el-form-item label="是否启用" prop="status">
          <el-radio-group v-model="form.status">
            <el-radio v-for="dict in emp_status" :key="dict.value" :label="parseInt(dict.value)">{{
              dict.label
            }}</el-radio>
          </el-radio-group>
        </el-form-item>
...
const data = reactive({
  ...
  rules: {
    ...
    regionId: [{ required: true, message: "负责区域不能为空", trigger: "blur" }],
...
function submitForm() {
  proxy.$refs["empRef"].validate((valid) => {
    if (valid) {
      //补全员工角色名称、角色编号与区域名称
      form.value.roleName = roleList.value.find(
        (item) => item.roleId == form.value.roleId,
      ).roleName;
      form.value.roleCode = roleList.value.find(
        (item) => item.roleId == form.value.roleId,
      ).roleCode;
      form.value.regionName = regionList.value.find(
        (item) => item.id == form.value.regionId,
      ).regionName;
