...
const total = ref(0);
const title = ref("");
const viewOpen = ref(false); // 点位详情对话框是否显示
...
const data = reactive({
  ...
  rules: {
    ...
    businessType: [{ required: true, message: "商圈类型不能为空", trigger: "change" }],
    regionId: [{ required: true, message: "区域ID不能为空", trigger: "blur" }],
    partnerId: [{ required: true, message: "合作商ID不能为空", trigger: "blur" }],
  },
});
